...
<span>Qiymet: ${x.price * x.count}<svg xmlns="http://www.w3.org/2000/svg" width="11" height="12"
        ...
        <div>
            <button class="plus_btn" onclick="inc(${x.id})">+</button>
            <button class="minus_btn" onclick="dec(${x.id})">-</button>
            <i onclick="trash(${x.id})" class="fa-solid fa-trash"></i>
...
function inc(i) {
    let checkBasket = basket.find(b => b.id === i)
    checkBasket.count += 1
    showBasket(basket)
    localStorage.setItem('sebet', JSON.stringify(basket))
...
function dec(d) {
    let checkBasket = basket.find(a => a.id === d)
    checkBasket.count -= 1
    showBasket(basket)
